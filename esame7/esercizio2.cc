...
// *********************************************
list* estrai_rec(tree* root, list* res)
{
    if(root == NULL) return res;

    if(root->left != NULL)
        res = estrai_rec(root->left, res);
    
    if(root->info%2 == 0)
        res = new list{root->info, res};
    
    if(root->right != NULL)
        res = estrai_rec(root->right, res);
